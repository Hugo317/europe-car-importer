"""MCP stdio server exposing finn.no (Norway) car listing search/pricing.
...
list_makes_models. See finn_client.py for the scraping/parsing logic.
"""
from mcp.server.fastmcp import FastMCP

import finn_client as fc
...
def search_listings(
    ...
    """Search finn.no (Norway) car listings, one page (up to ~49 results).
    ...
    """
    try:
        return fc.search_listings(make, model, year_from, year_to, page)
    except fc.FinnError as e:
        return {"error": str(e)}
...
def price_analysis(
    ...
    `max_listings` finn.no listings, in both NOK and EUR (via a live
    ECB reference rate).
    ...
    """
    try:
        return fc.price_analysis(make, model, year_from, year_to, max_listings)
    except fc.FinnError as e:
        return {"error": str(e)}
...
def list_makes_models(make: str, model: str | None = None) -> dict:
    """Resolve a make/model against finn.no's real taxonomy (make/series/model tree).

    Unlike otomoto-pl/standvirtual-pt's best-effort slug guessing, this
    enumerates finn.no's actual valid series/model list for the make, since
    ...
    """
    try:
        return fc.list_makes_models(make, model)
    except fc.FinnError as e:
        return {"error": str(e)}
